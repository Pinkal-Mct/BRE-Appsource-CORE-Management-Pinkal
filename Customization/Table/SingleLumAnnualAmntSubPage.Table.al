table 73209687 "BLRSingleLumAnnualAmntSubPage"
{
    ...
    fields
    {
        ...
        field(73209576; "BLRSL_Merged Unit ID"; Code[100])
        {
            ...
            begin
                if LeaseProposal.Get("BLRProposal ID", "BLRSL_Merged Unit ID") then begin
                    Rec."BLRSL_Start Date" := LeaseProposal."BLRLease Start Date";
                    Rec."BLRSL_End Date" := LeaseProposal."BLRLease End Date";
                    Rec."BLRSL_Unit Sq Ft" := LeaseProposal."BLRUnit Size";
        ...
        field(73209591; "BLRTotalFinalAmount"; Decimal)
        {
            ...
            CalcFormula = sum("BLRSingleLumAnnualAmntSubPage"."BLRSL_Final Annual Amount" where("BLRProposal ID" = field("BLRProposal ID")));
        }
        field(73209592; "BLRTotalAnnualAmount"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum("BLRSingleLumAnnualAmntSubPage"."BLRSL_Annual Amount" where("BLRProposal ID" = field("BLRProposal ID")));
        }
        field(73209593; "BLRTotalRoundOff"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum("BLRSingleLumAnnualAmntSubPage"."BLRSL_Round off" where("BLRProposal ID" = field("BLRProposal ID")));
        }
        field(73209594; "BLRTotalFirstAnnualAmount"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum("BLRSingleLumAnnualAmntSubPage"."BLRSL_Final Annual Amount" where("BLRProposal ID" = field("BLRProposal ID"), "BLRSL_Year" = const(1)));
        }
    }
    keys
    {
        key(PK;"BLRProposal ID", "BLRSL_Line No.")
        {
            Clustered = true;
        }
    }
